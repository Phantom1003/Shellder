# shellder — SSH ControlMaster daemon for macOS
...
|---|---|---|
| `-N` (default) | a connection with no session | cleanest; some servers reject it |
...
ones transparent; `Tools/menubar-template.swift`), so macOS tints it like its
...
every launch, at login included; `shellder --background` forces that for a
single launch.
...
* **Keychain prompts and rebuilds.** The login keychain checks two things
  ...
  (Settings → Accounts → Manage Certificates → + → Apple Development; a free
